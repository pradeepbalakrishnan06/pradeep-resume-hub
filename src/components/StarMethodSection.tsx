		...
		items: [
			{
				...
				action:
					"Led transitions using structured KT plans and HCL's ASSeT™ framework.",
				result:
			...
			return (
				<div className="space-y-4">
					{tab.items.map((item, index) => (
						<Collapsible
							key={index}
							className="border border-resume-soft-gray rounded-md overflow-hidden"
						>
							...
							<CollapsibleContent className="p-3 space-y-2 border-t border-resume-soft-gray/50">
								<div>
									<span className="font-semibold text-resume-dark-gray">
										Task:
									</span>
				...
				<table className="w-full">
					<thead>
						<tr className="bg-resume-soft-gray border-b">
							<th className="text-left p-4 text-resume-dark-gray font-semibold">
								Situation
							</th>
							<th className="text-left p-4 text-resume-dark-gray font-semibold">
								Task
					...
					<tbody>
						{tab.items.map((item, index) => (
							<tr
								key={index}
								className="border-b hover:bg-resume-soft-gray/20 transition-colors"
							>
								<td className="p-4 text-resume-dark-gray">
									{item.situation}
								</td>
		...
		<section id="star" className="py-12 md:py-24 bg-white">
			<div className="container mx-auto px-4">
				...
				<p className="text-sm md:text-lg mb-6 md:mb-8 max-w-3xl opacity-0 animate-fade-in animation-delay-1">
					Explore my key achievements and measurable impact across different domains
								...
								>
									{tabs.map((tab, index) => (
										<TabsTrigger
											...
											className={`
        data-[state=active]:bg-resume-terracotta
						...
						{tabs.map((_, index) => (
							<button
								...
								className={`h-2 w-2 rounded-full transition-colors duration-300 ${
									currentTab === index
										? "bg-resume-terracotta"
										: "bg-resume-light-gray"
								}`}
								onClick={() => setCurrentTab(index)}
							/>
						))}
